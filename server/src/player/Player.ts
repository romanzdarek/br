...
export class Player {
	...
	constructor(
		...
		const halberd = new Halberd(this, players, map, collisionPoints);
		const sword = new Sword(this, players, map, collisionPoints);
		const mace = new Mace(this, players, map, collisionPoints);

		this.inventory = new Inventory(this, loot, halberd, sword, mace);
	...
	acceptHit(power: number, attacker?: Player, weapon?: Weapon): void {
		...
		if (this.inventory.vest) {
			...
			if (
				weapon &&
				(weapon === Weapon.Pistol || weapon === Weapon.Rifle || weapon === Weapon.Shotgun || weapon === Weapon.Machinegun || weapon === Weapon.Grenade)
			) {
				power *= 0.67;
			}
		}
			...
			for (const loot of this.loot.lootItems) {
				...
				if (distance < lootAndPlayerRadius) {
					...
					if (loot.type === LootType.Pistol && (!this.inventory.item1 || !this.inventory.item2)) {
						...
						automaticTake = true;
					} else if ([LootType.Halberd, LootType.Sword, LootType.Mace].includes(loot.type) && (!this.inventory.item3 || !this.inventory.item33)) {
						automaticTake = true;
					...
					} else if (loot.type === LootType.Grenade && this.inventory.item4GrenadeCount < 3) {
						automaticTake = true;
	...
	loop(): void {
		...
		if (this.inventory.activeItem instanceof HandWeapon) {
		...
		if (this.mouseControll.left) {
			if (this.inventory.activeItem === Weapon.Hand) {
			...
			} else if (this.inventory.activeItem instanceof HandWeapon) {
				if (this.inventory.activeItem.ready()) {
					this.inventory.activeItem.hit();
					this.sounds.push(new Sound(SoundType.HandWeapon, this.getCenterX(), this.getCenterY()));
					this.mouseControll.left = false;
				}
			} else if (this.inventory.activeItem === Weapon.Grenade) {
				if (this.hands[1].throwReady()) {
					this.throw();

					this.Grenades.push(
						new Grenade(this, this.hands[1], this.mouseControll.x, this.mouseControll.y, this.sounds, this.touchControll.touchendDelay)
					);
					this.touchControll.touchendDelay = 0;
					this.mouseControll.left = false;
				}
				...
				if (this.hands[1].throwReady()) {
					...
					this.Grenades.push(new Smoke(this, this.hands[1], this.mouseControll.x, this.mouseControll.y, this.touchControll.touchendDelay));
					this.touchControll.touchendDelay = 0;
					this.mouseControll.left = false;
				}
					...
					for (let j = 0; j < this.collisionPoints.body.length; j++) {
						...
						if (rectangleObstacle.isPointIn(pointOnMyPosition)) {
							...
							if (!rectangleObstacle.hasFixedPosition()) {
								if (shiftX > 0) rectangleObstacle.shift(1, 0, this.map, this.players);
								if (shiftX < 0) rectangleObstacle.shift(-1, 0, this.map, this.players);
								if (shiftY > 0) rectangleObstacle.shift(0, 1, this.map, this.players);
								if (shiftY < 0) rectangleObstacle.shift(0, -1, this.map, this.players);
							}
							return false;
						}
		...
		for (let i = 0; i < this.map.roundObstacles.length; i++) {
			...
			if (roundObstacle.isActive()) {
				...
				if (distance < obstacleAndPlayerRadius) {
					if (roundObstacle instanceof Bush) {
