...
export class Controller {
    constructor() {
        ...
        this.model = new Model(this.keys, this.mouse, this.socket, this.serverClientSync, this.myHtmlElements, this.editor);
    ...
    socketController() {
        ...
        //startGame
        this.socket.on('startGame', (mapData) => {
        ...
        //createGame
        this.socket.on('createGame', (gameId, playerName) => {
        ...
        //joinGame
        this.socket.on('joinGame', (gameId, playerName) => {
            this.model.setGameId(gameId);
        ...
        //updateListOpenGames
        this.socket.on('updateListOpenGames', (openGames) => {
            ...
            if (openGames.length)
                noGame = false;
            el.mainMenu.games.disabled = noGame;
        });
        //update listOfPlayers in lobby
        this.socket.on('listOfPlayers', (playerNames) => {
            //title
        ...
        this.socket.on('collisionPoints', (body, hand, hammer) => {
        ...
        //uodate listOfMaps
        this.socket.on('listOfMaps', (maps) => {
        ...
        this.socket.on('createPlayer', (name) => {
            if (name) {
        ...
        this.socket.on('m', (mapObject, serverTime, data) => {
            const updateTime = this.serverClientSync.getDrawDelay() - (this.serverClientSync.getServerTime() - serverTime);
            //walls
            if (mapObject === 'w') {
    ...
    mouseController() {
        this.myHtmlElements.zoneSVG.addEventListener('mousemove', (e) => {
            if (e.x == undefined) {
        ...
        this.myHtmlElements.zoneSVG.addEventListener('mousedown', (e) => {
        ...
        this.myHtmlElements.zoneSVG.addEventListener('mouseup', (e) => {
    ...
    menuController() {
        ...
        el.mapSizeMenu.ok.addEventListener('click', () => {
            this.editor.changeSize(parseInt(document.getElementById('mapSizeValue').value));
        ...
        el.openMapMenu.ok.addEventListener('click', () => {
            const mapName = el.openMapMenu.maps.value;
        ...
        el.mapEditorMenu.save.addEventListener('click', () => {
            ...
            const mapName = el.mapEditorMenu.name.value;
        ...
        el.mainMenu.name.addEventListener('input', () => {
            let disabled = true;
            if (el.mainMenu.name.value.length)
                disabled = false;
            el.mainMenu.create.disabled = disabled;
            let disabled2 = true;
            if (el.mainMenu.games.value)
                disabled2 = false;
            if (!disabled && !disabled2)
                el.mainMenu.join.disabled = false;
        });
        ...
        el.mainMenu.create.addEventListener('click', () => {
            if (el.mainMenu.name.value.length) {
                const playerName = el.mainMenu.name.value;
                const selectedMap = el.mainMenu.maps.value;
        ...
        el.mainMenu.join.addEventListener('click', () => {
            if (el.mainMenu.name.value.length && el.mainMenu.games.value) {
                const playerName = el.mainMenu.name.value;
                const gameIndex = parseInt(el.mainMenu.games.value);
                if (this.model.isNameOk(playerName)) {
